...
from general_motion_retargeting.utils.smpl_csv import as_gmr_smplx_data, load_gvhmr_pt, read_smpl_csv
...
def load_smplx_file(smplx_file, smplx_body_model_path):
    ...
    smplx_output = body_model(
        betas=torch.tensor(smplx_data["betas"][:10]).float().view(1, -1), # (16,)
        ...
        expression=torch.zeros(num_frames, 0).float(),
...
def _load_global_smpl_params(params, fps, smplx_body_model_path):
    """Build the exact SMPL-X inputs used by GMR from canonical SMPL arrays."""
    smplx_data = as_gmr_smplx_data(params, fps)
    ...
    num_frames = smplx_data['pose_body'].shape[0]
    smplx_output = body_model(
        # GVHMR shape is constant in practice; use frame zero as the original loader did.
        betas=torch.tensor(smplx_data["betas"][0]).float().view(1, 10),
        global_orient=torch.tensor(smplx_data["root_orient"]).float(), # (N, 3)
        body_pose=torch.tensor(smplx_data["pose_body"]).float(), # (N, 63)
        transl=torch.tensor(smplx_data["trans"]).float(), # (N, 3)
        left_hand_pose=torch.zeros(num_frames, 45).float(),
...
def load_gvhmr_pred_file(gvhmr_pred_file, smplx_body_model_path, fps=None):
    params, resolved_fps = load_gvhmr_pt(gvhmr_pred_file, fps=fps)
    return _load_global_smpl_params(params, resolved_fps, smplx_body_model_path)


def load_smpl_csv_file(smpl_csv_file, smplx_body_model_path, expected_fps=None):
    params, fps = read_smpl_csv(smpl_csv_file, expected_fps=expected_fps)
    return _load_global_smpl_params(params, fps, smplx_body_model_path)
...
def get_gvhmr_data_offline_fast(smplx_data, body_model, smplx_output, tgt_fps=30):
    ...
    if tgt_fps < src_fps:
        ...
        # Do not relabel frames when no temporal upsampling was performed.
