...
The legacy command remains supported. For project data exchange, use the canonical
SMPL CSV pipeline below. CSV is the unified intermediate format because its 80
named columns are inspectable, language-independent, and make world/camera data
mix-ups visible; it is **not** GMR's native runtime representation. GMR still
converts SMPL parameters through the SMPL-X body model into its per-frame mapping
of body names to global positions and quaternions before solving robot IK.

```bash
# GVHMR world/global SMPL -> canonical CSV. --fps is required when the PT has no FPS metadata.
cd /path/to/GVHMR
python scripts/gvhmr_to_smpl_csv.py \
  --input /path/to/hmr4d_results.pt \
  --output motions/dance_smpl.csv \
  --fps 50

# Canonical CSV -> G1, retaining the established GMR pickle schema.
cd /path/to/GMR
python scripts/smpl_csv_to_robot.py \
  --input motions/dance_smpl.csv \
  --robot unitree_g1 \
  --output motions/dance_g1.pkl

# The same robot trajectory as NPZ (use --no_viewer for a headless batch run).
python scripts/smpl_csv_to_robot.py \
  --input motions/dance_smpl.csv \
  --robot unitree_g1 \
  --output motions/dance_g1.npz \
  --no_viewer
```

The CSV has one frame per row and exactly 80 columns in this order:
`fps`; `smpl_params_global_global_orient_0..2`;
`smpl_params_global_body_pose_0..62`;
`smpl_params_global_transl_0..2`; and
`smpl_params_global_betas_0..9`. Orientations are SMPL axis-angle rotation
vectors in radians (21 body joints for `body_pose`), translations are metres,
and betas are dimensionless. Values are copied unchanged from GVHMR's
`smpl_params_global` world/gravity-aligned coordinate data; camera-coordinate
parameters are never used. During GMR processing, the existing GVHMR adapter
maps positions from GVHMR coordinates to GMR's z-up convention as `(x, -z, y)`
and applies the same fixed rotation to joint orientations.

Data flow: `hmr4d_results.pt[smpl_params_global]` → canonical SMPL CSV → SMPL-X
forward kinematics → GMR body transforms → robot IK trajectory. The robot pickle
uses GMR's existing fields: scalar `fps`, `root_pos [T,3]` in metres,
`root_rot [T,4]` in xyzw order, `dof_pos [T,D]` in radians, plus its existing
optional `local_body_pos` and `link_body_list` fields. NPZ output deliberately
contains only the numeric fields that actually exist in that schema:
`fps`, `root_pos`, `root_rot`, and `dof_pos`; no new fields are inferred.
