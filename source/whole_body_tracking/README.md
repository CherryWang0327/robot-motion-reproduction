...
For the fixed ten-motion, single-seed baseline procedure (30k minimum, 100k maximum, plateau stopping), follow
[BASELINE_MANUAL.md](BASELINE_MANUAL.md).
...
For a local motion file, use `--motion_file` instead of `--registry_name`:

```bash
python scripts/rsl_rl/train.py --task=Tracking-Flat-G1-v0 \
--motion_file /absolute/path/to/motion.npz \
--headless --logger tensorboard --run_name local_motion
```

The two motion-source arguments are mutually exclusive. Local mode does not
contact W&B. Use a staged workflow before committing to a full run: 16 envs × 2
iterations for schema/CUDA validation, 512–1024 envs × 2000–5000 iterations for
candidate screening, then 4096 envs × 30000 iterations for the selected setup.

Monitor ETA, health, and convergence from CPU without competing for GPU:

```bash
python scripts/analyze_training_convergence.py \
  --log_dir logs/rsl_rl/g1_flat/<run_directory> \
  --max_iterations 30000 \
  --watch 60
```

Export the final reward curve, tracking-error curves, scalar CSV, and convergence JSON:
...
The default convergence check compares two consecutive 1000-iteration windows.
It requires finite metrics, mean episode length at least 495, reward change below
1%, and changes in six tracking-error metrics below 2%. This is a training-curve
plateau check, not a replacement for deterministic checkpoint evaluation. For a
final report, evaluate selected checkpoints with fixed seeds, corruption/events
disabled, and the same complete motion.
...
python scripts/rsl_rl/play.py --task=Tracking-Flat-G1-v0 \
  --wandb_path={wandb-run-path} --motion_file /absolute/path/to/motion.npz --video --headless
```

For a deterministic local-motion evaluation with a complete video and tracking-error report:

```bash
python scripts/rsl_rl/play.py --task=Tracking-Flat-G1-v0 \
  --motion_file /absolute/path/to/motion.npz \
  --load_run <run_directory> --checkpoint <checkpoint_file> \
  --video --headless
```

Evaluation starts at reference frame zero with observation corruption, events, and initial-state perturbations disabled.
By default it runs one complete reference motion and writes the video, `tracking_errors.csv`, and `summary.json` under
the checkpoint run's `evaluation/<motion_name>/` directory. Use `--evaluation_output` to select another directory.
