...
motion_source.add_argument("--motion_file", type=str, help="Absolute path to a local motion NPZ file.")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    if args_cli.motion_file is not None:
        ...
        local_motion = pathlib.Path(args_cli.motion_file).expanduser()
        if not local_motion.is_absolute():
            parser.error("--motion_file must be an absolute path")
        if not local_motion.is_file():
            parser.error(f"--motion_file does not exist or is not a file: {local_motion}")
        motion_file = str(local_motion)
