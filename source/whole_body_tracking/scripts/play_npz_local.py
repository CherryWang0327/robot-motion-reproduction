...
conda activate bydmmc_nogmr
python play_npz_local.py \
    --motion_path /data/BysanRL/data_goal/npz/GVHMR_gmr/demo
...
parser.add_argument(
    "--lock_camera",
    action="store_true",
    help="若开启，则每帧强制跟随机器人设置相机；默认关闭，便于鼠标自由缩放/旋转视角。",
)
# =========== newADD end ========
# =========== newADD end ========
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    # =========== newADD end ========
    # ======== newADD start======
    # 只在启动时设置一次初始相机，之后默认交给鼠标控制
    init_root = controller.motion.body_pos_w[controller.get_frame_ids()][0, 0].detach().cpu().numpy()
    sim.set_camera_view(init_root + np.array([2.0, 2.0, 0.5]), init_root)
    # =========== newADD end ========
    try:
        # Simulation loop
        while simulation_app.is_running():
            ...
            # ======== newADD start======
            # 只有明确要求锁定相机时，才每帧强制重设相机。
            # 默认关闭，这样鼠标滚轮/拖动才能正常控制视角。
            if args_cli.lock_camera:
