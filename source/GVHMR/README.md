...
### Export canonical SMPL CSV for GMR

GVHMR owns the first stage of the unified pipeline. Export the world-coordinate
`smpl_params_global` result (never the camera-coordinate parameters) as the
canonical 80-column, one-frame-per-row SMPL CSV, then pass that CSV to GMR:

```bash
cd /path/to/GVHMR
python scripts/gvhmr_to_smpl_csv.py \
  --input outputs/demo/tennis/hmr4d_results.pt \
  --output outputs/demo/tennis/tennis_smpl.csv \
  --fps 30

cd /path/to/GMR
python scripts/smpl_csv_to_robot.py \
  --input /path/to/GVHMR/outputs/demo/tennis/tennis_smpl.csv \
  --robot unitree_g1 \
  --output motions/tennis_g1.npz \
  --no_viewer
```

The CSV stores FPS, global orientation (axis-angle radians), 21 body-joint
axis-angle rotations, global translation in metres, and 10 dimensionless shape
coefficients. `--fps` may be omitted only when the PT contains consistent FPS
metadata.
