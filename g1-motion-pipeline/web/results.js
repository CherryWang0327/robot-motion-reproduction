...
async function main() {
  ...
  const videoHtml = videos.length
    ? `<h2>视频结果</h2><div class="video-grid">${videos.map((file) => `<figure class="video-card"><video controls playsinline preload="metadata" src="/api/file?path=${encodeURIComponent(file)}"></video><figcaption>${escapeHtml(file.split('/').pop())}</figcaption></figure>`).join('')}</div>`
