...
Run GVHMR on an input video and retain its world-space reconstruction plus SMPL export. Inspect the input, camera-space, and world-space videos before continuing.

Expected evidence:

- `hmr4d_results.pt`: reconstructed motion result;
- SMPL motion export for downstream conversion;
...
Use the exported SMPL motion to generate a Unitree G1 PKL. Record the native GMR replay, then inspect joint limits, orientation, contact behavior, and the visual plausibility of the G1 motion.
