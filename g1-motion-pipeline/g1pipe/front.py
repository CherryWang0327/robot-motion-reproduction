...
def build_from_video(video: Path, route: str, run_dir: Path, dry_run: bool = False) -> Path:
    ...
    # Keep this branch byte-for-byte compatible in meaning with the verified
    # March.md workflow: GVHMR PT -> Y-up AMASS -> --source-y-up .motion ->
    # MotionLib -> SMPL tracker, then a separate PyRoki G1 retarget.
    ...
    _run(motion_dir, [str(PROTO_PYTHON), "data/scripts/gvhmr_to_amass_npz.py", str(hmr), str(amass_npz), "--fps", "30"], PROTO_ROOT, dry_run)
