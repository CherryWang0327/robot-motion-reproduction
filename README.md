...
Input video
  │
  ├─ GVHMR PT → world-space human motion + standard SMPL CSV
  │
  ├─ Route A: SMPL CSV → GMR → Unitree G1 PKL → WBT NPZ
  │
  └─ Route B: SMPL CSV → AMASS (Y-up) → ProtoMotions MotionLib → PyRoki G1 PT → WBT NPZ
                                                        │
                                                  validation gate
                                                        │
...
## My data-interface and retargeting contribution

I implemented and documented the deployable, auditable interface between GVHMR and the two G1 retargeting routes:

- Exported GVHMR's native `hmr4d_results.pt` into one standard 80-column SMPL CSV contract: `fps`, `global_orient` (3), `body_pose` (63), `trans` (3), and `betas` (10).
- Made that CSV the shared input to both GMR and ProtoMotions/PyRoki. The ProtoMotions branch derives its AMASS NPZ from the CSV instead of consuming the GVHMR PT directly.
- Kept coordinate ownership explicit: the CSV preserves GVHMR's gravity-aligned Y-up world data; ProtoMotions performs the single recorded Y-up → Z-up transform with `--source-y-up`.
- Preserved the native PT as provenance evidence, so an export or retargeting issue can be reproduced and audited without treating a downstream robot asset as the source of truth.

The routes intentionally diverge only after the human-motion interface: GMR emits a G1 PKL, while ProtoMotions builds SMPL `.motion`/MotionLib PT and then a PyRoki G1 PT. Both are converted to a validated WBT robot-reference NPZ before offline policy preview or a separately approved training request.
